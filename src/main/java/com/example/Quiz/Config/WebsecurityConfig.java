    ...
    public AuthenticationManager authenticationManagerBean() throws Exception {
        ...
        protected void configure(HttpSecurity http) throws Exception{ // tam thoi tat csrf de server respone
            http.cors().and().csrf().disable()
                    .authorizeRequests().antMatchers("/auth/login","/auth/register","/loginFacebook","/auth/forgot_password").permitAll().
//                    antMatchers("/users").hasRole("ADMIN"). // phai co role admin moi dc vao
